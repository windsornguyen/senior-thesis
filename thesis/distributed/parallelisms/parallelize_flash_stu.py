...
def parallelize_flash_stu(
    ...
    if parallel_dims.tp_enabled:
        if job_config.distributed.parallelism.get("enable_async_tensor_parallel") and not job_config.kernel.compile:
        ...
        apply_tp(
            ...
            enable_async_tp=job_config.distributed.parallelism.get("enable_async_tensor_parallel"),
        )

    if job_config.activation_checkpoint.mode != "none":
        apply_ac(model=model, ac_config=job_config.activation_checkpoint)
    ...
    if parallel_dims.dp_shard_enabled or parallel_dims.cp_enabled:
        ...
        if parallel_dims.cp_enabled:
            logger.info("Applied Context Parallelism to the model!")
        
        if job_config.training.enable_cpu_offloading:
        ...
        apply_ddp(
            model=model,
            dp_mesh=world_mesh,
            enable_compile=job_config,
            enable_compiled_autograd=job_config.distributed.kernel.enable_compiled_autograd,
        )
...
def apply_tp(
    ...
    # Apply tensor + sequence parallelism to every Flash STU block
    for _, flash_stu_block in model.layers.items():
        layer_plan = {
            "stu_norm": SequenceParallel(),
            "stu": prep(
...
# TODO: Decide which selective op for activation checkpointing for Flash STU
NO_RECOMPUTE_OPS = {
    ...
    # For low-precision training, it's useful to always save the result of max,
    # since the absolute maximum is used to compute the scaling factor for quantization.
    torch.ops.aten.max.default,
}

def _apply_ac_to_flash_stu_block(module: nn.Module, ac_config: JobConfig) -> None:
    valid_ac_modes = "full", "selective"
    if ac_config.mode not in valid_ac_modes:
        raise ValueError(
            f"Invalid activation checkpointing mode: {ac_config.mode}. "
            "Valid modes: {valid_ac_modes}"
        )
    
    if ac_config.mode == "full":
        return dist_ckpt_wrapper(module, preserve_rng_state=False)
    
    assert ac_config.mode == "selective", f"Invalid activation checkpointing mode: {ac_config.mode}"
    use_op_sac = ac_config.selective_ac_option == "op"
    use_layer_sac = ac_config.selective_ac_option.isdigit()
    if not use_op_sac and not use_layer_sac:
        raise ValueError(
            f"Invalid selective activation checkpointing option: {ac_config.selective_ac_option}. "
...
def apply_ac(model: nn.Module, ac_config: JobConfig) -> None:
    """
    Apply activation checkpointing to the model.
    """
    for layer_id, flash_stu_block in model.layers.named_children():
        flash_stu_block = _apply_ac_to_flash_stu_block(flash_stu_block, ac_config)
...
def apply_compile(model: nn.Module) -> None:
    ...
    (after applying DP).
    """
    logger.info("Compiling each Flash STU block with torch.compile...")
    start = time.perf_counter()
    for layer_id, flash_stu_block in model.layers.named_children():
        flash_stu_block = torch.compile(flash_stu_block, mode="max-autotune", fullgraph=True)
        model.layers.register_module(layer_id, flash_stu_block)
    end = time.perf_counter()
    logger.info(f"Finished compiling each Flash STU block in {end - start:.4f} seconds.")
...
def apply_fsdp(
    ...
    for layer_id, flash_stu_block in model.layers.items():
        if pp_enabled:
...
def apply_ddp(model: nn.Module, dp_mesh: DeviceMesh, enable_compile: bool, enable_compiled_autograd: bool) -> None:
    """
