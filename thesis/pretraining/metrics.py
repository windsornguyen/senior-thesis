"""Adapted from https://github.com/pytorch/torchtitan/blob/main/torchtitan/metrics.py"""
...
from thesis.distributed import device_module, device_type
...
class DeviceMemoryMonitor:
    ...
    def _to_gib(self, memory_in_bytes):
        # NOTE: GiB (gibibyte) is 1024^3 bytes, GB is 1000^3 bytes
...
class TensorBoardLogger(BaseLogger):
    ...
    def __init__(self, log_dir: str, tag: str | None = None) -> None:
        self.tag = tag
...
def _get_metrics_rank(parallel_dims: ParallelDims) -> int:
    """
    Returns global rank 0 in non-PP configs, and returns the global
    rank of the 0th rank in the last pipeline stage when PP is enabled.
    """
...
def build_metric_logger(job_config: JobConfig, parallel_dims: ParallelDims, tag: str | None = None) -> BaseLogger:
    """
    Build an appropriate metric logger based on the job configurations.
    """
    ...
    if not should_log:
        logger.debug("Returning BaseLogger since should_log is False.")
        return BaseLogger()
