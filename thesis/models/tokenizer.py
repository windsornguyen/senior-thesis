...
class Tokenizer(abc.ABC):
    ...
    @abc.abstractmethod
    def get_token_offsets(self, text: str, tokens: list[str] | None = None) -> tuple[list[int], list[int]]:
...
class ByteTokenizer(Tokenizer):
    ...
    def decode(self, tokens: list[int]) -> str:
        byte_tokens = bytes([token for token in tokens if token < 256])
...
class TikTokenTokenizer(Tokenizer):
    ...
    def get_token_offsets(self, text: str, tokens: list[str] | None = None) -> tuple[list[int], list[int]]:
        ...
        substrs = [text[s:e] for s, e in zip(offsets, offsets[1:] + [None], strict=True)]
        return substrs, offsets
