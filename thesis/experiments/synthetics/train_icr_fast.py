...
from thesis.experiments.utils.assoc_scan.kernel_1d import associative_scan
...
class SpectralAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, *, debug: bool = False) -> torch.Tensor:
        ...
        gates_z = lrelu2(self.wg_z(Z.view(-1, h2))) + self.eps  # [B*H*L,1]
        gates_z = gates_z.view(B, H, L)  # [B,H,L]

        gated_Z = Z * gates_z[..., None, None]  # [B,H,L,h,h]

        gated_Z_flat = gated_Z.view(B * H, L, h2).transpose(1, 2).contiguous()  # [B*H,h2,L]
        gates_z_flat = gates_z.view(B * H, 1, L).expand(-1, h2, -1)  # [B*H,h2,L]  (stride‑0 on h2)

        Z_cumul_flat, gate_cumul_flat = associative_scan(gated_Z_flat, gates_z_flat)  # each [B*H,h2,L]

        Z_cumul = Z_cumul_flat.transpose(1, 2).view(B, H, L, h, h)  # [B,H,L,h,h]
        gate_cum = gate_cumul_flat[:, 0, :].view(B, H, L)  # [B,H,L]

        H = Z_cumul / (gate_cum[..., None, None] + self.eps)  # [B,H,L,h,h]

        Y = torch.einsum("bhtp,bhtpn->bhtn", q, H)  # [B,H,L,h]
        Y_attn = Y.permute(0, 2, 1, 3).reshape(B, L, D)  # (B, L, D)
    ...
    def stu_conv(self, filters: torch.Tensor, inputs: torch.Tensor) -> torch.Tensor:
        ...
        causal_base = lambda sig, ker: fftconvolve(sig, ker, mode="full")[..., : sig.shape[-1]]
    ...
    def combine_fn(
        ...
        Args:
            x: First leaf, (Z_x, g_x) where Z_x is [L, h, h] and g_x is [L].
            y: Second leaf, (Z_y, g_y) where Z_y is [L, h, h] and g_y is [L].

        Returns:
            Tuple (Z_new, g_new) with updated accumulations.
        """
        Z_x, g_x = x
        Z_y, g_y = y
    ...
    def scan_fn(self, Z_slice: torch.Tensor, g_slice: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            Tuple (Z_cumul, g_cumul) with shapes [L, h, h] and [L]
        """
    ...
    def batched_scan_fn(self, gated_Z: torch.Tensor, gates_z: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Run associative scan independently for every (B,H) stream, returning only Z and gate accumulators.
        ...
        Returns:
            tuple: (Z_cumul, gate_cumul) with shapes:
                - Z_cumul: [B, H, L, h, h]
                - gate_cumul: [B, H, L]
        """
        ...
        # Z_cumul_flat, gate_cumul_flat = Z_cumul_flat, gate_cumul_flat = associative_scan(
        #     xs=(gated_Z_flat, gates_z_flat), combine_fn=self.combine_fn, dim=-1, reverse=False, combine_mode="generic"
        # )
        Z_cumul_flat, gate_cumul_flat = Z_cumul_flat, gate_cumul_flat = associative_scan(gated_Z_flat, gates_z_flat)
