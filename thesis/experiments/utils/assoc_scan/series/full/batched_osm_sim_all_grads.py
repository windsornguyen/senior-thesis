...
def bwd_online_softmax_2pass_kernel(
    x_ptr, sim_ptr, grad_softmax_ptr, grad_sim_score_ptr, grad_x_ptr, grad_sim_ptr,
    ...
    # Second pass: Compute gradients for x and sim
    for k in range(0, tl.cdiv(seq_len, BLOCK_SIZE)):
        start = k * BLOCK_SIZE
        offs = tl.arange(0, BLOCK_SIZE)
        mask = offs < seq_len - start
        ...
        tl.store(grad_x_ptr + pid_b * grad_x_stride_b + pid_h * grad_x_stride_h + (start + offs) * grad_x_stride_l, gx, mask=mask)
...
class OnlineSoftmax2Pass(torch.autograd.Function):
    ...
    def forward(x: torch.Tensor, sim: torch.Tensor):
        ...
        sim_score = torch.empty_like(x)  # Shape: (batch_size, num_heads, seq_len)
        ...
        return softmax, sim_score

    @staticmethod
    def setup_context(ctx, inputs, output):
        x, sim = inputs
        softmax, sim_score = output
        ctx.save_for_backward(x, sim, softmax, sim_score)
        ctx.triton_dtype = dtype_map.get(x.dtype, tl.float32)
        ctx.shape = x.shape
        ctx.dtype = x.dtype

    @staticmethod
    def backward(ctx, grad_softmax, grad_sim_score):
        x, sim, softmax, sim_score = ctx.saved_tensors
        ...
        bwd_online_softmax_2pass_kernel[grid](
            x, sim, grad_softmax, grad_sim_score, grad_x, grad_sim,
            b, h, l,
            x.stride(0), x.stride(1), x.stride(2),
            sim.stride(0), sim.stride(1), sim.stride(2),
            grad_softmax.stride(0), grad_softmax.stride(1), grad_softmax.stride(2),
            grad_sim_score.stride(0), grad_sim_score.stride(1), grad_sim_score.stride(2),
...
if __name__ == "__main__":
    ...
    sim = torch.randn(2, 8, 64, dtype=torch.float64, device="cuda", requires_grad=True)
    ...
    softmax, sim_score = online_softmax_2pass(x, sim)
    ...
    num_bytes = x.numel() * x.element_size() * 6  # x, sim, softmax, sim_score, read x again, read sim again
