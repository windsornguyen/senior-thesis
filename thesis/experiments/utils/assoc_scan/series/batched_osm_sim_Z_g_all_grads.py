...
if __name__ == "__main__":
    ...
    ref_max_cumul, ref_norm_cumul, ref_v_cumul, ref_Z_cumul, ref_gate_cumul = batched_scan_fn(
        sim=sim, v=v, gated_Z=Z, gates_z=g_ref
    )

    ref_linear_attn = ref_v_cumul / (ref_norm_cumul[..., None] + 1e-6)
    ref_weights = torch.exp(sim - ref_max_cumul) / (ref_norm_cumul + 1e-6)
    ref_H = ref_Z_cumul / (ref_gate_cumul.unsqueeze(-1).unsqueeze(-1) + 1e-6)
    ...
    Y_base = torch.einsum("bhtp,bhtpn->bhtn", q, Z_cumul / (gate_cumul.unsqueeze(-1).unsqueeze(-1) + 1e-6))
    print(f"Y_base vs. ref_Y_base max absolute error: {(Y_base - ref_Y_base).abs().max().item()}")

    print(f"Z_cumul vs. ref_Z_out[-1] max absolute error: {(Z_cumul - ref_Z_cumul[:, :, -1:]).abs().max().item()}")
    print(f"sim_score vs. ref_norm_cumul[-1] max absolute error: {(sim_score - ref_norm_cumul[:, :, -1:]).abs().max().item()}")
    print(f"gate_cumul vs. ref_g_out[-1] max absolute error: {(gate_cumul - ref_gate_cumul[:, :, -1:]).abs().max().item()}")
