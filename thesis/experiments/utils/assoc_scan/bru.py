...
Can be expressed a tril matvec:
y = sum from j=0 to i (x[j])
  = L @ x

- Each output y_i is sum of the first i+1 elements of inputs x.

---

Gradient flow:

We wanna compute grad of loss L wrt to inputs x given outputs y.

This is expressed as:

dL/dx = dL/dy * dy/dx      # Chain with output and input
      = (dy/dx)^T * dL/dy  # Transpose of Jacobian    (1)
      = L^T * dL/dy        # Substitute dy/dx = L     (2)

See that:
y = L @ x => dy/dx = L
          => Gradient wrt inputs is the TRANSPOSE of (1)

Therefore, we compute the gradients by transposing, which turns a forward pass into a backward pass.
- If assoc_scan(x) in fwd accumulates from left-to-right,
- Then assoc_scan(grad_out[::-1])[::-1] in reverse pass accumulates from right-to-left!
...
def vjp_combine(
    ...
    z_y, # block<float, H*H>
    g_y: float,
    H: tl.constexpr, # Need H for block operations
    H2: tl.constexpr, # Need H*H for block operations
    DTYPE: tl.constexpr, # Need DTYPE for block operations
) -> Tuple[float, float, tl.tensor, tl.tensor, float, float, float, tl.tensor, tl.tensor, float]:
    # Recompute forward combine components for Jacobian calculation
    m_new = tl.maximum(m_x, m_y)
    # Handle potential inf values for stability in exp
    m_x_safe = tl.where(tl.isinf(m_x), tl.full_like(m_x, -1e20), m_x) # Use a large negative number
    m_y_safe = tl.where(tl.isinf(m_y), tl.full_like(m_y, -1e20), m_y)
    m_new_safe = tl.maximum(m_x_safe, m_y_safe)
    exp_x = tl.exp(m_x_safe - m_new_safe)
    exp_y = tl.exp(m_y_safe - m_new_safe)

    # ---------- Partial derivatives of S_new wrt S_x and S_y ----------
    # These are elements of the Jacobian matrices J(S_new, S_x) and J(S_new, S_y)

    # d(m_new)/d(m_x)
    dm_new_dm_x = tl.where(m_x >= m_y, 1.0, 0.0)
    # d(m_new)/d(m_y)
    dm_new_dm_y = 1.0 - dm_new_dm_x # tl.where(m_x < m_y, 1.0, 0.0)

    # d(s_new)/d(m_x)
    ds_new_dm_x = s_x * exp_x * (1.0 - dm_new_dm_x) - s_y * exp_y * dm_new_dm_x
    # d(s_new)/d(s_x)
    ds_new_ds_x = exp_x
    # d(s_new)/d(m_y)
    ds_new_dm_y = -s_x * exp_x * dm_new_dm_y + s_y * exp_y * (1.0 - dm_new_dm_y)
    # d(s_new)/d(s_y)
    ds_new_ds_y = exp_y

    # d(n_new)/d(m_x) - This is a vector
    dn_new_dm_x = n_x * exp_x * (1.0 - dm_new_dm_x) - n_y * exp_y * dm_new_dm_x
    # d(n_new)/d(n_x) - This is a scalar factor times identity
    dn_new_dn_x = exp_x
    # d(n_new)/d(m_y) - This is a vector
    dn_new_dm_y = -n_x * exp_x * dm_new_dm_y + n_y * exp_y * (1.0 - dm_new_dm_y)
    # d(n_new)/d(n_y) - This is a scalar factor times identity
    dn_new_dn_y = exp_y

    # d(Z_new)/d(Z_x) = Identity
    # d(Z_new)/d(Z_y) = Identity

    # d(g_new)/d(g_x) = 1.0
    # d(g_new)/d(g_y) = 1.0

    # ---------- Compute P_new = P_x @ J(S_new, S_x) + P_y @ J(S_new, S_y) ----------
    # P_x and P_y are row vectors (gm, gs, gn, gZ, gg)
    # J(S_new, S_x) and J(S_new, S_y) are Jacobian matrices

    # gm_new = sum over k (P_x[k] * d(S_new,k)/d(m_x)) + sum over k (P_y[k] * d(S_new,k)/d(m_y))
    # Considering the sparsity:
    gm_new = (
        gm_x * dm_new_dm_x
        + gs_x * ds_new_dm_x
        + tl.dot(gn_x, dn_new_dm_x.to(tl.float32), H) # Dot product for vector part
        + gm_y * dm_new_dm_y
        + gs_y * ds_new_dm_y
        + tl.dot(gn_y, dn_new_dm_y.to(tl.float32), H) # Dot product for vector part
    )

    gs_new = (
        gm_x * 0.0 # d(m_new)/d(s_x)
        + gs_x * ds_new_ds_x
        + tl.dot(gn_x, tl.zeros([H], dtype=tl.float32), H) # d(n_new)/d(s_x) is 0 vector
        + gm_y * 0.0 # d(m_new)/d(s_y)
        + gs_y * ds_new_ds_y
        + tl.dot(gn_y, tl.zeros([H], dtype=tl.float32), H) # d(n_new)/d(s_y) is 0 vector
    )
    # simplified:
    gs_new = gs_x * ds_new_ds_x + gs_y * ds_new_ds_y


    # gn_new is a block (vector)
    # gn_new[k] = sum over j (P_x[j] * d(S_new,j)/d(n_x,k)) + sum over j (P_y[j] * d(S_new,j)/d(n_y,k))
    # d(n_new,j)/d(n_x,k) = exp_x * delta_jk
    gn_new = (
        gm_x * tl.zeros([H], dtype=tl.float32) # d(m_new)/d(n_x) is 0 vector
        + gs_x * tl.zeros([H], dtype=tl.float32) # d(s_new)/d(n_x) is 0 vector
        + gn_x * dn_new_dn_x # element-wise multiplication by scalar exp_x
        + gm_y * tl.zeros([H], dtype=tl.float32) # d(m_new)/d(n_y) is 0 vector
        + gs_y * tl.zeros([H], dtype=tl.float32) # d(s_new)/d(n_y) is 0 vector
        + gn_y * dn_new_dn_y # element-wise multiplication by scalar exp_y
    )
    # simplified:
    gn_new = gn_x * dn_new_dn_x + gn_y * dn_new_dn_y

    # gZ_new is a block (matrix)
    # d(Z_new)/d(Z_x) is identity matrix. gZ_x is row vector. gZ_x @ I = gZ_x
    gZ_new = gz_x + gz_y # d(Z_new)/d(Z_x) and d(Z_new)/d(Z_y) are identity matrices

    # gg_new is a float
    gg_new = gg_x + gg_y # d(g_new)/d(g_x) and d(g_new)/d(g_y) are 1.0

    # Recompute the new forward state
    ...
    return (
        gm_new,
        gs_new,
        gn_new.to(DTYPE), # Ensure output dtype matches
        gZ_new.to(DTYPE), # Ensure output dtype matches
        gg_new,
        m_new,
        s_new,
        n_new.to(DTYPE), # Ensure output dtype matches
        z_new.to(DTYPE), # Ensure output dtype matches
        g_new,
    )
...
if m.dtype != torch.float64:
    ...
    for comp_name, loss_fn in components:
        ...
        m_out, s_out, n_out, Z_out, g_out = associative_scan(
            m_triton, s_triton, n_triton, Z_triton, g_triton
        )
