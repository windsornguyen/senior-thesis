...
Can be expressed as a tril matvec:
...
dL/dx = dL/dy * dy/dx      # Chain rule: backprop through function
      = (dy/dx)^T * dL/dy  # Reverse-mode: multiply by Jacobian transpose (1)
      = L^T * dL/dy        # Substitute dy/dx = L for prefix sum (2)

See that:
y = L @ x => dy/dx = L
          => dL/dx = L^T @ dL/dy

Therefore, we compute the gradients by transposing, which turns a forward scan into a backward scan:
...
This is equivalent to computing the **vector-Jacobian product (VJP)**:
- We are applying the **transpose of the Jacobian** of the forward combine function to the upstream gradients.
- The VJP defines how gradients of the loss w.r.t. outputs (dL/dy) propagate to inputs (dL/dx).
- In the context of associative scans, this becomes another scan — but in reverse — with a "backward combine" that encodes the transposed Jacobian.
"""
...
@triton.jit
def vjp_combine_fn(
    # ----- gradients flowing from the LEFT (up-stream) ----------------
    gm_l: float, gs_l: float, gn_l: float, gZ_l: float, gg_l: float,
    # ----- forward state of the LEFT input ----------------------------
    m_l: float,  s_l: float,  n_l: float,  Z_l: float,  g_l: float,
    # ----- gradients flowing from the RIGHT carry ---------------------
    gm_r: float, gs_r: float, gn_r: float, gZ_r: float, gg_r: float,
    # ----- forward state of the RIGHT input ---------------------------
    m_r: float,  s_r: float,  n_r: float,  Z_r: float,  g_r: float,
):
    # 1. forward combine -------------------------------------------------
    m_max  = tl.maximum(m_l, m_r)
    exp_l  = tl.exp(m_l - m_max)
    exp_r  = tl.exp(m_r - m_max)

    m_new  = m_max
    s_new  = s_l * exp_l + s_r * exp_r
    n_new  = n_l * exp_l + n_r * exp_r
    Z_new  = Z_l + Z_r
    g_new  = g_l + g_r

    # 2. Jacobian pieces -------------------------------------------------
    sel_r  = (m_r >= m_l).to(m_l.dtype)
    sel_l  = 1.0 - sel_r

    dsm_r  = s_r * exp_r * (1 - sel_r) - s_l * exp_l * sel_r
    dnm_r  = n_r * exp_r * (1 - sel_r) - n_l * exp_l * sel_r   # vector
    dss_r  = exp_r
    dnn_r  = exp_r                                             # vector

    # 3. VJP for the CURRENT (left) element -----------------------------▼
    gm_here = gm_r * sel_l + gs_r * dsm_r + gn_r * dnm_r
    gs_here = gs_r * dss_r
    gn_here = gn_r * dnn_r          # remains a vector
    gZ_here = gZ_r
    gg_here = gg_r

    # 4. accumulate upstream gradient -----------------------------------▼
    gm_new = gm_l + gm_here
    gs_new = gs_l + gs_here
    gn_new = gn_l + gn_here
    gZ_new = gZ_l + gZ_here
    ...
    # 5. return (new_carry , new_state) ----------------------------------
    return (gm_new, gs_new, gn_new, gZ_new, gg_new,
            m_new,  s_new,  n_new,  Z_new,  g_new)
...
@triton.autotune(
    configs=[conf for conf in get_configs() if filter_config(conf)],
    key=["batch_size", "feature_size_h", "seq_len"],
)
@triton.jit
def backward_scan_kernel(
    ...
    offset_rev = base_offset + rev_indices * stride_l
    offset_fwd = base_offset + indices * stride_l

    # Load forward inputs in reverse order
    max_val = tl.load(max_ptr + offset_rev, mask=mask, other=-1e20)
    sum_val = tl.load(sum_ptr + offset_rev, mask=mask, other=0.0)
    num_val = tl.load(num_ptr + offset_rev, mask=mask, other=0.0)
    z_val = tl.load(z_ptr + offset_rev, mask=mask, other=0.0)
    gate_val = tl.load(gate_ptr + offset_rev, mask=mask, other=0.0)

    # Load output gradients in reverse order
    grad_max_out = tl.load(grad_out_max_ptr + offset_rev, mask=mask, other=0.0)
    grad_sum_out = tl.load(grad_out_sum_ptr + offset_rev, mask=mask, other=0.0)
    grad_num_out = tl.load(grad_out_num_ptr + offset_rev, mask=mask, other=0.0)
    grad_z_out = tl.load(grad_out_z_ptr + offset_rev, mask=mask, other=0.0)
    grad_gate_out = tl.load(grad_out_gate_ptr + offset_rev, mask=mask, other=0.0)

    # --- Perform VJP scan (associative combine) ---
    grad_m, grad_s, grad_n, grad_Z, grad_g, _, _, _, _, _ = tl.associative_scan(
        (
            # 5 gradient tensors
            grad_max_out,
            grad_sum_out,
        ...
        axis=0,
        combine_fn=vjp_combine_fn,
        reverse=False,
    )

    # Store gradients
    tl.store(grad_in_max_ptr + offset_rev, grad_m, mask=mask)
    tl.store(grad_in_sum_ptr + offset_rev, grad_s, mask=mask)
    tl.store(grad_in_num_ptr + offset_rev, grad_n, mask=mask)
    tl.store(grad_in_z_ptr + offset_rev, grad_Z, mask=mask)
    tl.store(grad_in_gate_ptr + offset_rev, grad_g, mask=mask)
...
class AssociativeScan(torch.autograd.Function):
    ...
    def backward(ctx, grad_m, grad_s, grad_n, grad_Z, grad_g):
        ...
        print('here', grad_in_m[..., :2, 0, 0])

        grad_m_final = grad_in_m.sum(dim=(-1, -2)) / h2       # shape [..., L]
        grad_s_final = grad_in_s.sum(dim=(-1, -2)) / h2
        grad_g_final = grad_in_g.sum(dim=(-1, -2)) / h2

        grad_n_final = grad_in_n.sum(dim=-1) / H              # shape [..., L, H]

        grad_Z_final = grad_in_Z                              # already correct
